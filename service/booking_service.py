...
class BookingService:
    ...
    def add(self, hotel_id, room_id, customer_id, number_peoples: str, check_in, check_out):
        ...
        if (book_hotel and book_room and book_customer and validate_date(check_in) and number_peoples.isdigit() and
                0 <= int(number_peoples) <= book_room.capacity and validate_date(check_out) and
                check_dates(check_in, check_out)):
            if book_room.status == "available" and not book_customer.is_banned:
                book_room.status = "booked"
                return self.repo.add(Booking(book_room, book_hotel, book_customer, number_peoples, check_in, check_out))
